"""
Improved DOCX parser v8.
Fixes:
1. Marker detection: Use (?:^|\s) instead of look-behind for fixed-width compatibility.
2. Choice splitting: Refined to handle multiple markers on one line.
3. Reading comprehension: Proper passage repetition.
"""
...
def has_any_marker(text):
    # Marker must be at start or preceded by space/tab
...
def split_choices(text, bold_text):
    ...
    # Split by markers. We use finditer to find all markers and their positions
    ...
    for p in paras:
        ...
        if is_question_start(text):
            ...
            if has_any_marker(text):
                ...
                # If the line has markers, extract the text before markers as the question
                m = re.search(r'(?:^|\s)[A-D][\.\)]\s*', text)
                q_part = text[:m.start()].strip()
                if q_part: curr_q = [q_part]
