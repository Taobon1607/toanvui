...
def is_question_start(text):
    # 'Câu 1:', 'Question 1.', '1.' (only if followed by space and text)
...
def parse_docx(paras, topic_id):
    ...
    for p in paras:
        ...
        if re.match(r'^(Choose|Read the following|Mark the|Hãy chọn|Chọn)', text, re.I):
            ...
            instruction = text
            passage = []
            in_reading = 'reading' in text.lower() or 'following' in text.lower() or 'đoạn văn' in text.lower()
            continue
        ...
        if has_choice_marker(text):
            ...
            # If starts with A. or no choices yet, start new
        ...
        if in_reading and not curr_q:
            # Check if this plain text might be choices without markers
            # (only if we have 4 lines following a question)
            passage.append(text)
        elif curr_choices:
            save()
            curr_q = [text]
